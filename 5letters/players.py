...
#создает окно приложения и настраивает его с помощью метода setupUi.
#sock,которая хранит сокет для обмена данными с сервером
class Player1(QMainWindow, player1.Ui_MainWindow):
    ...
    def keyPressEvent(self, event):
        if event.key() == 16777220:
            self.check()
#проверяет, является ли длина слова, введенного пользователем, равной 5.
#Если это так, то слово приводится к нижнему регистру и отправляется на сервер через сокет.
    def check(self, ):
        word = self.word.text()
        if len(word) != 5:
